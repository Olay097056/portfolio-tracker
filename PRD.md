...
- ~~`error`/`loading` ไม่ได้ reset พร้อม `points` ในบล็อก render-phase เดียวกัน~~ — **แก้แล้วใน Ticket 2**: เพิ่ม `setError(null)` เข้าไปใน render-phase block เดียวกับ `setPoints(null)` แล้ว มี test ยืนยัน (child-component probe pattern เพราะ naive test แบบเช็ค `result.current.error` ตรงๆ กลายเป็น false-positive — effect เดิม clear error อยู่แล้วไม่ว่า fix จะมีหรือไม่มี)
- ~~`_fetch_from_provider` hardcode `row.Index.strftime("%Y-%m-%d")` ใช้ได้แค่กับ daily/weekly bar~~ — **แก้แล้วใน Ticket 2**: `ChartPoint.time` widen เป็น `str | int`, intraday range (1D/5D) เข้ารหัสเป็น UNIX timestamp, range อื่นยังเป็น date string ตามเดิม มี test ยืนยันทั้งสอง encoding + test ยืนยันว่า int รอดผ่าน Pydantic serialization จริง (ไม่ใช่แค่ mock)
- **`useChartData.ts` ใช้ `useRef` แทน `useState` สำหรับเก็บ ticker+range ก่อนหน้า** — ยังไม่แก้ (ไม่ได้อยู่ใน scope ของ Ticket 2) pattern ที่ React docs แนะนำสำหรับ "reset state เมื่อ prop เปลี่ยนระหว่าง render" ใช้ `useState` เพราะ ref mutation ไม่ถูก roll back ถ้า React discard render นั้นทิ้ง (เช่นถ้าใช้ `startTransition`/`Suspense` ในอนาคต) — ตอนนี้ยังไม่มี concurrent feature ไหนในโปรเจกต์เลยเลยไม่มีผลจริง แต่เป็น footgun แฝงถ้าเพิ่มในอนาคต
