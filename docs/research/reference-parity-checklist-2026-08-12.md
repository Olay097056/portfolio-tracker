...
| 2.2 | ✅ มี | ความมั่นใจต่อโมเดล (%) | คำนวณจริง = % indicators มีข้อมูลสด (model_service.py:889) แต่ตอนนี้ 100 ทุกตัว | เสร็จ (ใบ 11 — ModelsDashboard:340 + OverviewDashboard:274; ไม่แตะ Boardroom/Signals ที่เป็นคนละค่า) |
...
| 7.2 | ✅ มี | "ข้อมูลจำกัด/รายวัน/เรียลไทม์" badge ต่อประเทศ | แสดงแล้ว (CountriesDashboard.tsx:167 data_tier_note_th) แต่เป็นข้อความธรรมดา · tier 4 ระดับ (sparse/daily/realtime/manual) vs ต้นฉบับ 3 | เสร็จ (ใบ 11 — ui/DataTierBadge 4 tier: realtime เขียว/daily ฟ้า/sparse เหลือง/manual เทา; 27 badge บนหน้าจริง) |
...
| 9.4 | ✅ มี | UX details — ref มี filter ประชุมล้มเหลว/แยก type | มี StatusBadge (BoardroomDashboard.tsx:243) + list แต่ไม่มี filter · backend พร้อม (trigger_type) | เสร็จ (ใบ 11 — filter ที่ server ก่อน .limit(50); แยก state ไม่ให้แผงประชุมสดหาย) |
| **สรุป** | | **4 มี · 0 ขาด · 0 ต่าง** | |
...
| 12.2 | ✅ มี | ref: impact slider · เรา: dropdown (NewsDashboard.tsx:201,279-289 — ฟังก์ชันครบ) | เสร็จ (ใบ 11 — range 0-100 step 5, ยิง fetch ตอน pointerUp) |
