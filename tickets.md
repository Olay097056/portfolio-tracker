...
**Resolution (2026-08-09, user-confirmed):** `country_ai_service.py` — brief (AI สรุปสถานการณ์: brief_md + คำแนะนำ + จินตนาการ) + report (รายงานเชิงลึก 15 หัวข้อ markdown) both via DeepSeek (`deepseek-v4-flash`, max_tokens 8000, response_format json_object, temperature 0.2 — reuse news_service's key/URL; load_dotenv added for standalone runs). Prompt fed with live country context (10Y, full curve, fx, bps-vs-US, score+components). Stored in SQLite `country_briefs`/`country_reports` (model_used, generated_at). Endpoints: `GET /api/countries/{code}/brief` (auto-generate on page open, cached 24h, stale copy fallback, 503 on failure) + `POST /api/countries/{code}/report` (on-demand like the reference). User approved sample: TH brief 13s (3 recommendations + 3 scenarios, real data only), TH report 37s / 15 sections — "ไม่มีข้อมูล" written when data missing, never fabricated. Tests: 4 new (brief generated+cached, cached reuse no-regenerate, report on demand, failure→503) — full suite 461 passes. Live in docker: brief 200 with real DeepSeek content.

- [x] Prototype: one country (TH) — brief (สรุปสถานการณ์ + คำแนะนำ + จินตนาการ scenarios) from current yield/fx/news data; deep report (~14 หัวข้อ) on demand; measure tokens/time (max_tokens 8000 lesson)
- [x] Show the user a sample brief + report to judge tone/length before wiring all 27
- [x] Decide storage: SQLite `country_briefs` / `country_reports` (generated_at, model_used) + on-demand regeneration
- [x] News panel: which 6 news items per country (reuse news_service's impact scores; country tag needed)
