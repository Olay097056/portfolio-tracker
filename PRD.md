...
Ticket "Dashboard price chart walking skeleton" merge เข้า master แล้ว หลัง final review 3 รอบ (รอบ 1 เจอ Critical 1 + Important 2 เรื่อง, รอบ 2 เจอว่า fix รอบแรกของ Critical ไม่พอ — `key`-based remount เฉยๆยังเหลือช่องที่กราฟใหม่โดน feed ข้อมูล ticker เก่า, รอบ 3 ยืนยันว่า fix ที่ถูกต้อง (reset `points` แบบ render-phase ใน `useChartData` แทนที่จะพึ่ง remount อย่างเดียว) ปิดช่องได้จริง — trace มือ + reproduce RED/GREEN ยืนยันแล้ว) residual ที่เหลือค้างเป็น Minor ที่ round 3 ตัดสินใจไม่บล็อก merge:

- **`useChartData.ts` ใช้ `useRef` แทน `useState` สำหรับเก็บ ticker ก่อนหน้า** — pattern ที่ React docs แนะนำสำหรับ "reset state เมื่อ prop เปลี่ยนระหว่าง render" ใช้ `useState` เพราะ ref mutation ไม่ถูก roll back ถ้า React discard render นั้นทิ้ง (เช่นถ้าใช้ `startTransition`/`Suspense` ในอนาคต) — ตอนนี้ยังไม่มี concurrent feature ไหนในโปรเจกต์เลยเลยไม่มีผลจริง (grep แล้วไม่เจอ `startTransition|useDeferredValue|Suspense|lazy` ที่ไหน) แต่เป็น footgun แฝงถ้าเพิ่มในอนาคต — สลับเป็น `useState` ใช้เวลาไม่นาน
- **`error`/`loading` ไม่ได้ reset พร้อม `points` ในบล็อก render-phase เดียวกัน** — กราฟใหม่ (chart line) ไม่โดน stale data แล้ว แต่ error text อาจโชว์ของ ticker เก่าค้างอยู่ 1 เฟรมก่อน effect ของ `useChartData` รันจริง — impact ต่ำ (ข้อความ ไม่ใช่กราฟ) test ไม่ทันจับเพราะ `act()` flush effect แบบ sync ใน test environment
- **`_fetch_from_provider` hardcode `row.Index.strftime("%Y-%m-%d")` ใช้ได้แค่กับ daily/weekly bar** — Ticket 2 (range selector) จะเพิ่ม range 1D→5min และ 5D→30min ซึ่งจะมีหลาย point ในวันเดียวกัน ชน `time` key ซ้ำกันใน `lightweight-charts` (ถือเป็น duplicate business-day key) กราฟ intraday จะยุบข้อมูลหาย — **ต้องแก้ point encoding เป็น UNIX timestamp สำหรับ interval ระดับนาที ก่อนเริ่ม implement Ticket 2** ไม่ใช่แค่เติม mapping table แถวใหม่เฉยๆตามที่ comment ใน `chart_service.py` สัญญาไว้ (สัญญานั้นผิด ต้องแก้ `ChartPoint.time` เป็น `str | int` ด้วย)
