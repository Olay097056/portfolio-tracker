...
- **พลิกสถานะแถวใน checklist เมื่อไหร่ ต้องอัปเดตทุกตัวสรุปที่นับแถวนั้นในคอมมิตเดียวกัน แล้วพิสูจน์ด้วยสคริปต์นับสด** — เอกสารนี้มีสรุปซ้อน 3 ชั้น (สรุปย่อยรายหมวด → Grand Summary → map + ใบ 01) — แก้ทีละชั้นไม่มีวันจบ (พลาดมา 4 ชั้น: แถว → Grand Summary → สรุปย่อย 17 บรรทัด → ใบ 01 gap/5-16) · **หลักฐานต้องใช้โหมดอ่านอย่างเดียว**: `python3 .scratch/reference-parity/fix_subsummaries.py --check` (โหมดเขียนเป็นตัวแก้ ไม่ใช่ตัวตรวจ; `--check` ต้อง exit 1 เมื่อจงใจทำไฟล์พัง)
...
- **สถิติที่ต้องรอไม้ปิด (ยกมาจากแผน `trade-desk-ui-100` ที่ยกเลิก 2026-08-12)** — `max_drawdown` + วันที่ peak→trough · `total_fees` · `total_tokens` / `total_llm_calls` · per-turn `latency_s`/`success_count`/`fail_count`/`reject_count` · **ทุกตัวคำนวณจากไม้ที่ปิดแล้ว และ prod มี `closed_count = 0`** — สร้างตอนนี้ได้หน้าจอที่ขึ้น "—" ทุกช่องและพิสูจน์ไม่ได้ว่าสูตรถูก (เหตุผลเดียวกับที่ใบ 05 เลื่อน hit rate) · `extended_stats` ที่เหลือมีครบแล้วบน prod
- **TA signals + TIER column ในตารางตลาด** (ยกมาจากแผนเดียวกัน) — ไม่เคยผ่านการตัดสินของ user และ algorithm ยังไม่นิยาม (แผนเดิมเขียนไว้ใน "Not yet specified" ของตัวเอง) — ต้อง grilling ก่อนถ้าจะเอา
